...
struct ContentView: View {
    ...
    @State private var editingEntry: SweepEntry?
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Chimney Sweep Log")
            .toolbar {
...
struct EntryFormView: View {
    ...
    init(mode: EntryFormMode) {
        ...
        if case .edit(let entry) = mode {
            ...
            _inspectorNotes = State(initialValue: entry.inspectorNotes)
    ...
    var body: some View {
        NavigationStack {
            Form {
                DatePicker("Sweep Date", selection: $date, displayedComponents: .date)
                    .accessibilityIdentifier("entryDateField")
                TextField("Inspector Notes", text: $inspectorNotes)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    ...
                    .disabled(inspectorNotes.trimmingCharacters(in: .whitespaces).isEmpty)
                }
            }
        }
    }
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = SweepEntry(date: date, inspectorNotes: inspectorNotes, notes: notes)
            store.add(entry)
        case .edit(let existing):
            var updated = existing
            updated.date = date
            updated.inspectorNotes = inspectorNotes
            updated.notes = notes
